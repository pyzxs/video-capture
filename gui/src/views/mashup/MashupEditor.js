...
import { generatedApi, materialApi, folderApi } from '../../api/index.js'
  ...
  setup() {
    ...
    function drawToCanvas(seekVideo = true) {
      ...
      for (let t = trackLines.value.length - 1; t >= 0; t--) {
        ...
        for (const clip of trackLines.value[t].list) {
          if (f >= clip.start && f < clip.end) clips.push(clip)
        }
      }
    ...
    const addToTimeline = (m) => {
      ...
      if (!inserted) {
        ...
        line.list = [{ ...clip }]
        // Insert video/audio after main, others before
    ...
    const addTextToTimeline = () => {
      ...
      if (!targetLine) {
        ...
        trackLines.value.splice(trackLines.value.length - 1, 0, targetLine)
      }
