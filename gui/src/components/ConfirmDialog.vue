<template>
  <Teleport to="body">
    <div v-if="state.confirmVisible" class="modal-overlay">
      <div class="modal confirm-modal">
        <div class="modal-header">
          <h3>{{ state.confirmTitle }}</h3>
          <button class="btn btn-default" @click="cancel" title="取消">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
        <p class="confirm-message">{{ state.confirmMessage }}</p>
        <div class="modal-actions">
          <button class="btn btn-danger" @click="confirmAction">
            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
            确定
          </button>
        </div>
      </div>
    </div>
  </Teleport>
</template>

<script>
import { useToast } from '../composables/useToast.js'

export default {
  name: 'ConfirmDialog',
  setup() {
    const { state, confirmAction, cancel } = useToast()
    return { state, confirmAction, cancel }
  },
}
</script>

<style scoped>
.confirm-modal { width: 400px; }
.confirm-message {
  font-size: 14px;
  color: var(--text-secondary);
  margin: 8px 0 0;
  line-height: 1.6;
}
.confirm-modal .modal-actions {
  justify-content: flex-end;
  margin-top: 24px;
}
</style>
