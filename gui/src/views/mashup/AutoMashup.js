...
export default {
  ...
  setup() {
    ...
    const { play: playOne } = usePlaybackGuard()
    ...
    const autoForm = ref({ description: '', ratioIdx: '', voice: '', batchCount: 1, skipExpand: true, audioMaterialId: '', showSubtitle: false })
    ...
    const startAuto = async () => {
      ...
      const basePayload = {
        ...
        show_subtitle: autoForm.value.showSubtitle,
    ...
    // ── Generated video play / delete ──
    const playingGenId = ref(null)
    const genVideoRefs = {}
    const setGenVideoRef = (id, el) => { if (el) genVideoRefs[id] = el }

    const playGeneratedVideo = (v) => {
      if (playingGenId.value === v.id) {
        const el = genVideoRefs[v.id]
        if (el) el.pause()
        playingGenId.value = null
        return
      }
      // pause previous
      if (playingGenId.value && genVideoRefs[playingGenId.value]) {
        genVideoRefs[playingGenId.value].pause()
      }
      playingGenId.value = v.id
      const el = genVideoRefs[v.id]
      if (el) playOne(el)
    }

    const onGenVideoEnded = (id) => {
      if (playingGenId.value === id) playingGenId.value = null
    }

    const deleteGeneratedVideo = async (v, idx) => {
      if (!confirm('确定删除该混剪视频？')) return
      try {
        await generatedApi.remove(v.id)
        autoGeneratedVideos.value.splice(idx, 1)
        if (playingGenId.value === v.id) playingGenId.value = null
        toast.success('已删除')
      } catch (e) {
    ...
    return {
      ...
      truncate, apiUrl,
      playingGenId, setGenVideoRef, playGeneratedVideo, onGenVideoEnded, deleteGeneratedVideo,
    }
  },
}
