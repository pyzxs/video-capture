...
def create_app() -> FastAPI:
    ...
    def startup():
        ...
        try:
            ...
            # Preserve auth fields that exist only in config.json
            current = _load()
    ...
    def _import_config_to_db(db):
        ...
        SETTING_META = {
            "llm_model": ("探索大模型", "LLM 模型名称", 1, 0),
            "llm_provider": ("探索大模型", "LLM 供应商", 1, 0),
            "asr_model_size": ("语音转文本", "ASR 模型大小", 0, 0),
            "asr_api_model": ("语音转文本", "ASR 模型名称", 1, 0),
            "whisper_model_dir": ("语音转文本", "Whisper 模型目录", 0, 0),
            "vector_db_path": ("embedding", "向量数据库路径", 0, 0),
            "embedding_model": ("embedding", "Embedding 模型名称", 1, 0),
            "tts_model": ("文本转语音", "TTS 模型名称", 1, 0),
            "tts_voice": ("文本转语音", "TTS 音色", 1, 0),
            ...
            "log_level": ("系统设置", "日志级别", 0, 0),
            "log_dir": ("系统设置", "日志目录", 0, 0),
            "cms_base_url": ("系统设置", "CMS 服务器地址", 1, 0),
